...
function RegisterPage() {
  ...
  const inputClass = (fieldName) =>
    `w-full bg-gray-700 border ${
      errors[fieldName] ? 'border-red-500 focus:border-red-400' : 'border-gray-600 focus:border-indigo-500'
    } text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 ${
      errors[fieldName] ? 'focus:ring-red-500/50' : 'focus:ring-indigo-500'
    ...
    <div className="min-h-screen bg-gray-900 flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-indigo-950 to-gray-900 items-center justify-center p-12">
        ...
        <div className="relative text-center">
          ...
          <h2 className="text-3xl font-bold text-white mb-3">Join CampusConnect</h2>
          <p className="text-gray-400 text-lg leading-relaxed max-w-sm">
            Create your free account and start building connections with your campus community.
          </p>
          <div className="mt-10 grid grid-cols-2 gap-3">
            {[
              { icon: '🎓', label: 'Student Profiles' },
              { icon: '🔍', label: 'Team Finder' },
              { icon: '🛒', label: 'Marketplace' },
              { icon: '🔗', label: 'Connections' },
            ].map((item) => (
              <div
                key={item.label}
                className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-gray-300 text-sm"
              >
                <span>{item.icon}</span>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-400">Join your campus community today — it's free.</p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  value={formData.role}
                  onChange={handleChange}
                  className="w-full bg-gray-700 border border-gray-600 focus:border-indigo-500 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-colors"
                >
                  <option value="student">🎓 Student</option>
                  <option value="committee">📢 Committee Head</option>
                </select>
                {formData.role === 'committee' && (
                  <p className="text-amber-400 text-xs mt-1.5">
                    Committee Heads can broadcast events to all students in their college.
                  </p>
                )}
              </div>
              ...
              <div className="bg-indigo-900/20 border border-indigo-700/30 rounded-xl p-3">
                <p className="text-indigo-300 text-xs leading-relaxed">
                  💡 Use your college or university email (e.g. .edu, .ac.in) to verify your student
                  status and unlock all features.
                </p>
              </div>
